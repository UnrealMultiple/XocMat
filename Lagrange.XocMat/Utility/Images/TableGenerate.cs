...
public class TableStyle
{
    ...
    public int MaxAutoWidth { get; set; } = 2000;
    ...
    public int CellPaddingTop { get; set; } = 8;
    public int CellPaddingBottom { get; set; } = 8;
    ...
    public float MinColumnWidthRatio { get; set; } = 0;  // 列最小宽度比例(相对于表格总宽度)
    public float MaxColumnWidthRatio { get; set; } = 2f;  // 列最大宽度比例(相对于表格总宽度)
    public bool EnableTextWrapping { get; set; } = false;    // 即使自动调整列宽，也允许长文本换行
    public int MaxTextLines { get; set; } = 20;   
}
...
public class TableGenerate
{
    ...
    private int CalculateMaxRowHeight(Font tableFont)
    {
        ...
        foreach (var row in TableRows)
        {
            ...
            foreach (var cell in row.Cells)
            {
                ...
                int cellHeight = (int)size.Height 
        ...
        return maxHeight / Math.Max(TableRows.Count, 1);
    }
    ...
    // 不再限制最大宽度，完全按内容需求
    return columnWidths.Values.Sum(); // 增加安全边距
}
    ...
    private int DrawTable(IImageProcessingContext context, Font tableFont, int currentY, int cardX, int cardWidth)
    {
        ...
        for (int rowIndex = 0; rowIndex < TableRows.Count; rowIndex++)
        {
            ...
            {
                if (cellIndex >= maxColumns) break; // 防止数组越界
                ...
                {
                    textOptions.WrappingLength = columnWidth - TableStyle.CellPaddingLeft - TableStyle.CellPaddingRight;
