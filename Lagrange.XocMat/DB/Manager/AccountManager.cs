...
public class AccountManager
{
    ...
    public AccountManager()
    {
        ...
        var table = new SqlTable("Account",
            ...
            new SqlColumn("Group", MySqlDbType.Text) { DefaultValue = XocMatAPI.Setting.DefaultPermGroup }
            );
    ...
    private List<Account> GetAccounts()
    {
        ...
        while (read.Read())
        {
            ...
            accounts.Add(new Account(ID, XocMatAPI.GroupManager.GetGroupNullDefault(GroupName)));
        }
        return accounts;
    }
    ...
    public void AddAccount(long userid, string group)
    {
        ...
        if (!XocMatAPI.GroupManager.HasGroup(group))
            throw new AccountException($"组 {group} 不存在，无法添加!");
        var exec = database.Query("INSERT INTO `Account` (`ID`, `Group`) VALUES (@0, @1)", userid, group);
        if (exec == 1)
        {
            Accounts.Add(new Account(userid, XocMatAPI.GroupManager.GetGroupNullDefault(group)));
        }
        else
        {
    ...
    public Account GetAccountNullDefault(long userid)
    {
        return Accounts.Find(x => x.UserId == userid)
            ?? new Account(userid, XocMatAPI.GroupManager.GetGroupNullDefault(""));
    }
    ...
    public void ReAccountGroup(long userid, string Group)
    {
        ...
        {
            GetAccountNullDefault(userid).Group = XocMatAPI.GroupManager.GetGroupNullDefault(Group);
        }
        else
        {
